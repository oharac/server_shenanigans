...
A standard `lapply` on 4 elements also takes about 4 times as long:

```{r lapply}
system.time({
  out_list <- lapply(X = test_vec, FUN = do_stuff)
})
 #   user  system elapsed 
 # 15.479   1.500  16.969 
dplyr::bind_rows(out_list)
...
A parallelized `mclapply` on 4 elements, across 4 cores, runs in about the same time as the single function:

```{r four cores}
system.time({
  out_list <- parallel::mclapply(X = test_vec, FUN = do_stuff, mc.cores = 4)
})
### on Aurora
#  user  system elapsed 
# 0.008   0.024   4.199 
...
More elements, more cores:

```{r twenty cores}
system.time({
  out_list <- parallel::mclapply(X = 1:20, FUN = do_stuff, mc.cores = 20)
})
### on Aurora
#  user  system elapsed 
# 0.008   0.024   4.199 
...
### Parallelization notes and etiquette

Time: $T_{total} = \frac{N_{processes}}{N_{cores}}\times t_{process}$

Memory: $Mem_{total} = Mem_{process} \times N_{cores}$ (sort of...)

* Each process/iteration must be independent - result does not depend on the output of another iteration
* More cores means faster: the server can be working on each process in parallel, at the same time
* BUT: each core requires memory

Aurora has a 💩-ton of cores and a 💩-ton of memory, but also lots of other users who want to access it, so be polite:

* don't hog all the cores, don't hog all the memory, don't crash the server
* use `htop` to see if there are currently people using a lot of cores and/or memory, and check in with them
* use `htop` to test complex parallelization functionality on a small sample to make sure it is working properly before setting it loose on the server

Note also: sometimes a slow function can be improved without parallelization!  use profiling to identify time-consuming or memory-consuming processes within your code...
